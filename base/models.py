...
class User3rdInfo(models.Model):
    ...
    type = models.IntegerField('type',choices=[(1,u"qq"),(2,u"weixin"),(3,u'weibo')],default=1)
    ...
    access_updated_at = models.DateField('access_updated_at',blank=True,default=datetime.now().date())
    ...
    expires_in = models.DateField('expires in',blank=True)
    re_expires_in = models.DateField('expires in',blank=True)
    refresh_token = models.CharField('refresh token', max_length=100, blank=True)
    verified = models.IntegerField('verified', blank=True)
    created_at = models.DateField('created at',default=datetime.now().date())
    updated_at = models.DateField('updated at',default=datetime.now().date())
    class Meta:
