...
class TuijianViews(LsView):
    def get(self,request, source_uid,version='',page=1,*args, **kwargs):
        ...
        docs=Document.objects.filter(source_uid__exact=source_uid).filter(topic__status__exact=1).filter(topic__topic_type__exact=2).order_by('-topic__created_at')[pageInfo.startNum:pageInfo.endNum]
